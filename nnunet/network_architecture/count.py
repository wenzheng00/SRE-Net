...
model = Generic_UNet(1, 16, 4, 3, True, False, lambda x: x, InitWeights_He(1e-2),True, True, True).to(device)
...
macs,params = get_model_complexity_info(model,(1, 40, 56, 40),as_strings=True,print_per_layer_stat=True)
# params = sum(p.numel() for p in model.parameters() if p.requires_grad)
# print('params:', params)
print('macs:', macs,'   params:', params)

inputs = torch.randn(9, 1, 40, 56, 40).to(device)
